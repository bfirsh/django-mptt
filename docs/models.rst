...
.. admonition:: About this document
   ...
   This document describes the abstract model Django MPTT provides so you can
...
``mptt.Model`` - the abstract MPTT Django model
===============================================

The ``mptt`` module contains a ``Model`` class, which is an abstract model that provides all the fields and methods required for MPTT.

.. _`minimal example usage`:

A mimimal example usage of ``mptt.Model`` is given below, where the
model being set up for MPTT is suitable for use with the default
field and tree manager attributes::
...
Model instance methods provided by ``mptt.Model``
=================================================
...
accessed through is specified by the ``tree_manager_attr`` ``TreeMeta`` 
option..
...
If ``commit`` is ``True``, ``node``'s ``save()`` method will be called
